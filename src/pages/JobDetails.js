...
const JobDetails = () => {
  ...
  const { id } = useParams();
  // pollingInterval page refresh na korei real time a updated data dekhaye dei
  const { data, isLoading } = useJobByIdQuery(id, { pollingInterval: 500 });
  ...
  const handleApply = () => {
    if (user.role === "employeer") {
      toast.error("You need a candidate account to apply", { id: "jobApplyButton" })
      return;
    }
    if (user.role === "") {
      navigate("/register");
  ...
  return (  
    <div className='pt-14 flex-none lg:flex lg:flex-col'>
